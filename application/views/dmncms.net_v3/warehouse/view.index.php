        ...
        <div class="box-style1" style="margin-bottom: 20px;">
            ...
            <div class="entry">
                    ...
                    <div class="form" id="sell_item">
                        <form method="post" action="<?php echo $this->config->base_url; ?>warehouse"
                            ...
                            <table>
                                    ...
                                    <td>
                                        <select class="custom-select" name="payment_method" id="payment_method">
                                            <?php if ($this->config->config_entry('warehouse|allow_sell_for_credits') == 1) { ?>
                                                <option
                                                    value="1"><?php echo $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_1'); ?></option><?php } ?>
                                            <?php if ($this->config->config_entry('warehouse|allow_sell_for_gcredits') == 1) { ?>
                                                <option
                                                    value="2"><?php echo $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_2'); ?></option><?php } ?>
                                            <?php if ($this->config->config_entry('warehouse|allow_sell_for_zen') == 1) { ?>
                                                <option
                                                    value="3"><?php echo $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_3'); ?></option><?php } ?>
                                ...
                                ?>
                                <tr>
                                    <td><label for="highlight" id="highlight_info"
                                               data-info="<?php echo vsprintf(_('You Item Will Be Highlighted In Market. Price %d %s.'), array($this->config->config_entry('market|price_highlight'), $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_1'))); ?>"><?php echo _('Highlight Item'); ?></label>
                                    </td>
                                ...
                                <tr>
                                    <td></td>
                                    <td>
                                        <button type="submit" id="sell_item_button"><?php echo _('Sell Items'); ?></button>
                                    </td>
                                </tr>
                            </table>
                        </form>
                ...
                if ($this->config->config_entry('warehouse|allow_delete_item') == 1): ?>
                    <div class="i_note"><?php echo _('Right mouse click if you want to delete item.'); ?></div>
					<div style="clear:both;"></div>
                    <?php
                endif;
                    $wh_content = '<div class="wh_items" style="background-image: url(' . $this->config->base_url . 'assets/' . $this->config->config_entry('main|template') . '/images/wh.jpg)" id="wh_content">' . "\n";
                    for ($i = 1; $i <= 120; $i++) {
                        if (isset($items[$i])) {
                    ...
                    $wh_content .= '</div>';
                    echo $wh_content;
                    $wh_content2 = '<div class="wh_items" style="background-image: url(' . $this->config->base_url . 'assets/' . $this->config->config_entry('main|template') . '/images/wh.jpg)">';
