...
<div id="content">
    <div id="box1">
        <div class="title1">
            <h1><?php echo _('Shop Cart'); ?></h1>
        </div>
        <div class="box-style1" style="margin-bottom: 20px;">
            <h2 class="title"><?php echo _('My Shop Cart Items'); ?></h2>
            ...
            <div class="entry">
                ...
                <div id="credits_items">
                    ...
                    if ($credits_items != false) {
                        ...
                        <form method="POST" name="items_credits" id="items_credits" action="">
                            <table class="add_to_card" id="credits_table" cellspacing="0">
                                ...
                                foreach ($credits_items as $key => $items) {
                                    $i++;
                                    $even = ($i % 2) ? 'class="even"' : '';
                                    $this->iteminfo->item_data($items['item_hex']);
                                    ?>
                                    <tr id="item_<?php echo $items['id']; ?>" <?php echo $even; ?>>
                                        <td><span id="card_item_<?php echo $i; ?>"
                                                  data-info="<?php echo $items['item_hex']; ?>"><a
                                                    href="#"><?php echo $this->iteminfo->GetName(true); ?></a></span>
                                        </td>
                                        <td><span
                                                id="price_1_<?php echo $items['id']; ?>"><?php echo $items['price'] . '</span> ' . $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_1'); ?>
                                        </td>
                                        <td><input type="checkbox" id="add_to_warehouse_1_<?php echo $items['id']; ?>"
                                                   name="add_to_warehouse[<?php echo $items['id']; ?>]"/></td>
                                        <td><a href=""
                                               id="remove_item_1_<?php echo $items['id']; ?>"><?php echo _('Remove'); ?></a>
                                        </td>
                                    </tr>
                                    <?php
                                }
                            ...
                            <div style="padding-top:10px;">
                                <div
                                    style="padding:3px 13px;float:left;"><?php echo _('Total Price'); ?>
                                    : <span id="total_price_c"
                                            style="color: red;">0</span> <?php echo $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_1'); ?>
                                </div>
                                <div style="float:right;">
                                    <button id="buy_items_credits"
                                            class="custom_button"><?php echo _('Buy Selected'); ?></button>
                                </div>
                            </div>
                        </form>
                        ...
                        ?>
                        <div
                            class="i_note"><?php echo sprintf(_('No items for %s in cart.'), $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_1')); ?></div>
                        <?php
                    }
                    ?>
                </div>
                ...
                <div id="gcredits_items">
                    ...
                    if ($gcredits_items != false) {
                            ...
                            <table class="add_to_card" id="gcredits_table" cellspacing="0">
                                ...
                                foreach ($gcredits_items as $key => $items) {
                                    ...
                                    <tr id="item_<?php echo $items['id']; ?>" <?php echo $even; ?>>
                                        ...
                                        <td><span
                                                id="price_2_<?php echo $items['id']; ?>"><?php echo $items['price'] . '</span> ' . $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_2'); ?>
                                        </td>
                                        <td><input type="checkbox" id="add_to_warehouse_2_<?php echo $items['id']; ?>"
                                                   name="add_to_warehouse[<?php echo $items['id']; ?>]"/></td>
                                        <td><a href=""
                                               id="remove_item_2_<?php echo $items['id']; ?>"><?php echo _('Remove'); ?></a>
                                        </td>
                                    </tr>
                                    <?php
                                }
                            ...
                            <div style="padding-top:10px;">
                                <div
                                    ...
                                    : <span id="total_price_gc"
                                            style="color: red;">0</span> <?php echo $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_2'); ?>
                                </div>
                                <div style="float:right;">
                        ...
                        ?>
                        <div
                            class="i_note"><?php echo sprintf(_('No items for %s in cart.'), $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_2')); ?></div>
                        <?php
                    }
                    ?>
                </div>
