                    ...
                    foreach ($server_list AS $key => $server) {
                        ...
                        <tr id="<?php echo $key; ?>">
                            ...
                            <td>
                                ...
                                <?php else: ?>
                                    <a class="btn btn-success" href="#" id="server_status_button_<?php echo $key; ?>"
                                       onclick="App.changeServerStatus('<?php echo $key; ?>', 1);">
                                        <i class="icon-edit icon-white"></i>
                                        Enable
                                    </a>
                                <?php endif; ?>
                                <a class="btn btn-warning" href="#" onclick="App.deleteServer('<?php echo $key; ?>');">
                                    <i class="icon-edit icon-white"></i>
                                    Remove
                                </a>
                ...
                <form class="form-inline">
                    <fieldset>
                        <div class="control-group">
                            <div class="controls">
                                <label class="radio">
                                    <input type="radio" name="multiple_accoun_db_status" value="0" data-no-uniform="true"
                                           onclick="App.setUseMultiAccountDB(0);" <?php if (!$this->website->is_multiple_accounts()) {
                                        echo 'checked';
                                    } ?>>
                                    Accounts are loaded from first server in server list
                                </label>

                                <div style="clear:both"></div>
                                <label class="radio">
                                    <input type="radio" name="multiple_accoun_db_status" value="1" data-no-uniform="true"
                                           onclick="App.setUseMultiAccountDB(1);" <?php if ($this->website->is_multiple_accounts()) {
                                        echo 'checked';
                                    } ?>>
                                    Accounts are loaded from each server in server list
                                </label>
                            </div>
                        </div>
                    </fieldset>
