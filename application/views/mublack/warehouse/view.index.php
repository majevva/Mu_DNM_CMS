            ...
            <div class="entry">
                ...
                <?php
                    ...
                    <?php
                        ...
                        for($i = 1; $i <= 120; $i++){
                            if(isset($items[$i])){
                                $wh_content .= '<div id="item-slot-' . $i . '" class="square" style="margin-top:' . ($items[$i]['yy'] * 32) . 'px; margin-left:' . ($items[$i]['xx'] * 32) . 'px; width:' . ($items[$i]['x'] * 32) . 'px; background-image: url(' . $this->config->base_url . 'assets/' . $this->config->config_entry('main|template') . '/images/wh_root_on.png); height:' . ($items[$i]['y'] * 32) . 'px;" data-info="' . $items[$i]['hex'] . '"><img src="' . $this->itemimage->load($items[$i]['item_id'], $items[$i]['item_cat'], $items[$i]['level'], 0) . '" /></div>' . "\n";
                            }
                            else{
                        ...
                        if($this->website->get_value_from_server($this->session->userdata(['user' => 'server']), 'wh_size') > 1920){
                            ...
                            foreach($items AS $key => $item){
                                $wh_content2 .= '<div id="item-slot-' . $key . '" class="square" style="margin-top:' . ($item['yy'] * 32) . 'px; margin-left:' . ($item['xx'] * 32) . 'px; width:' . ($item['x'] * 32) . 'px; background-image: url(' . $this->config->base_url . 'assets/' . $this->config->config_entry('main|template') . '/images/wh_root_on.png); height:' . ($item['y'] * 32) . 'px;" data-info="' . $item['hex'] . '"><img src="' . $this->itemimage->load($item['item_id'], $item['item_cat'], $item['level'], 0) . '" /></div>' . "\n";
                            }
                            $wh_content2 .= '</div>';
                            echo $wh_content2;
                        }
