...
<div id="content">
    <div id="box1">
        ...
        <div class="box-style1" style="margin-bottom: 20px;">
            <h2 class="title"><?php echo _('Grand Reset your character'); ?></h2>
                                ...
                                <td>
                                    <?php if ($data['gres_info'] != false) {
                                        if ($this->session->userdata('vip')){
                                            $data['gres_info']['bonus_credits'] += $this->session->userdata(array('vip' => 'grand_reset_bonus_credits'));
                                            $data['gres_info']['bonus_gcredits'] += $this->session->userdata(array('vip' => 'grand_reset_bonus_credits'));
                                        }
                                    ?>
                                ...
                                <td>
                                    ...
                                    if ($data['gres_info'] != false) {
                                        if ($data['gres_info']['bonus_credits'] > 0) {
                                            $reward .= $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_1') . ' ' . _('Bonus') . ' + ' . $data['gres_info']['bonus_credits'] . '<br />';
                                        }
                                        if ($data['gres_info']['bonus_gcredits'] > 0) {
                                            $reward .= $this->config->config_entry('credits_' . $this->session->userdata(array('user' => 'server')) . '|title_2') . ' ' . _('Bonus') . ' + ' . $data['gres_info']['bonus_gcredits'] . '<br />';
                                        }
                                        if ($data['gres_info']['bonus_points_save'] == 1) {
                                            $reward .= _('Stats Bonus') . ' + ' . $this->Mcharacter->bonus_points_by_class($data['Class'], 'gres_info', $data) * ($data['gresets'] + 1) . '<br />';
                                        } else {
                                            $reward .= _('Stats Bonus') . ' + ' . $this->Mcharacter->bonus_points_by_class($data['Class'], 'gres_info', $data) . '<br />';
                                        }
                                        if ($data['gres_info']['bonus_reset_stats'] == 1 && $data['bonus_reset_stats'] > 0) {
                                            $reward .= _('Bonus Stats For Resets') . ' + ' . $data['bonus_reset_stats'] . '<br />';
                                        }
                                    }
                                    ?>
                                    <a class="view_reward" href="#"
                                       data-info="<?php echo $reward; ?>"><?php echo _('View Reward'); ?></a>
                                </td>
                                <td>
                                    <?php if ($data['gres_info'] != false) { ?><a href="#"
                                                                                  id="greset-char-<?php echo bin2hex($name); ?>"><?php echo _('Grand Reset'); ?></a><?php } else {
                                        echo _('Grand Reset Disabled');
                                    } ?>
                                </td>
                            </tr>
                            <?php
