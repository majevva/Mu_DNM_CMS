...
var App = {
	...
	initialize: function () {
		...
		$(".game_gnb a").on('mouseover', function(e){
			if($(this).data("menu")){
				$('#'+$(this).data("menu")).slideDown(200);
				$(this).addClass('on');
			}
		});
		$(".game_gnb a").on('mouseleave', function(e){
			if($(this).data("menu")){
				var related = $(e.relatedTarget);
				
				if (related.is('[id]')) {
					if(related.attr('id') != $(this).data("menu")){
						console.log(related.attr('id'));
						setTimeout(function () {
							$('#'+$(this).data("menu")).slideUp(200);
						}, 300);
					}
				}
				else if(related.is('[class]')){	
					if(related.attr('class') != 'game_list'){
						setTimeout(function () {
							$('#pull_server_list').slideUp(200);
							$('#pull_user_menu').slideUp(200);
							$('#pull_language_list').slideUp(200);
							$('#game_menu').removeClass('on');
		...
		$('#pull_server_list').on("mouseleave", function (e) {	
			if($(e.relatedTarget).data("menu") && $(e.relatedTarget).data("menu") == 'pull_server_list'){
				return ;
			}
			var menu = $(this);
			setTimeout(function () {
		...
		$('#pull_user_menu').on("mouseleave", function (e) {	
			if($(e.relatedTarget).data("menu") && $(e.relatedTarget).data("menu") == 'pull_user_menu'){
				return ;
			}
			var menu = $(this);
			setTimeout(function () {
		...
		$('#pull_language_list').on("mouseleave", function (e) {	
			if($(e.relatedTarget).data("menu") && $(e.relatedTarget).data("menu") == 'pull_language_list'){
				return ;
			}
			var menu = $(this);
			setTimeout(function () {
	...
	initializeModalBoxes: function () {
		...
		$('a[data-modal-div*=login_box]').leanModal({ top: 110, overlay: 0.45, closeButton: ".close_login" });	
	},
	...
	events: function () {
		...
		for (i in App.events_time) {
			$("#events").append($('<div id="event_info"><div id="info"><div id="channel">'+App.events_time[i].name+'</div></div><div class="timez" id="event'+App.events_time[i].id+'">'+App.formatedTime(App.events_time[i].left)+'</div></div>'));
			i++;
		}
		setInterval(function () {
